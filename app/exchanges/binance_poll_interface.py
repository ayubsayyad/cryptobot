...
class BinancePollInterface:
    ...
    def create(self):
        try:
            self.client = Client(self.message.client_details.Client_Api_Key,
                                 self.message.client_details.Client_Api_Key2, testnet=self.message.IsTestNet)
    ...
    def send_order(self, crypto_symbol, side, order_type, time_in_force, qty, price):
        try:
            ...
            order = self.client.create_order(symbol=crypto_symbol, side=side, type=order_type,
